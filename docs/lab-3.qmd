...
packages <- c("tidyverse", "here", "readxl", "scales", "RColorBrewer", "leaflet",
              ...
              "rnaturalearthdata", "readr", "kableExtra")
...
These two visualizations together might show that countries who 

# Comparing the Years


## Card 4 - Comparing 2018 and 2020 opinions (Quick facts) {.tabset}
```{r}
...
wgm18_Q21 <- wgm18 |> 
  ...
  mutate(
    ...
    question = case_when(
      str_detect(question, pattern = "Q21") ~ "Trust in government health and medical advice (Q21)",
      str_detect(question, pattern = "Q11B") ~ "Level of trust in national government (Q11B)"
    )
  )
...
Answer choices for Questions 11B and 21 are a lot, some, not much, or not at all. The percentages include those who responded with a lot or some trust. Answer choices for the 2020 questions are extremely important, somewhat important, not too important, not important at all, or don't know/refused to answer. The percentages include those who responded with extremely important or somewhat important.
